...
int main(int argc, char** argv) {
    ...
    PerformanceOption *performanceOption = new PerformanceOption(P);

    double prix;
    double ic;

    MonteCarlo *mc = new MonteCarlo(testModel, performanceOption, timestep, n_samples);

    mc->price(prix, ic);

    std::cout << "Prix de l'option en zero = " << prix << std::endl;
    std::cout << "Prix attendu : 1.257353 " << std::endl;
